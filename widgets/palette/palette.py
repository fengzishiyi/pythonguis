...
class _PaletteBase(QWidget):
    ...
    def _emit_color(self, color):
        self.selected.emit(color) #信号传递值
    ...
    def __init__(self, colors, *args, **kwargs):
        ...
        if isinstance(colors, str):
            if colors in PALETTES:
                colors = PALETTES[colors] # 键值

            palette = self.layoutvh()

            for c in colors:
                b = _PaletteButton(c)
                b.pressed.connect(lambda c=c: self._emit_color(c)) # 信号条件
                palette.addWidget(b)

            self.setLayout(palette)

class PaletteHorizontal(_PaletteLinearBase):
    layoutvh = QHBoxLayout # palette = self.QHBoxLayout()


class PaletteVertical(_PaletteLinearBase):
    layoutvh = QVBoxLayout # palette = self.QVBoxLayout()
...
class PaletteGrid(_PaletteBase):
    def __init__(self, colors, n_columns=5, *args, **kwargs):
        ...
        for c in colors:
            ...
            palette.addWidget(b, row, col) # 初始坐标(0, 0)
            col += 1
            if col == n_columns:
                col = 0
                row += 1
